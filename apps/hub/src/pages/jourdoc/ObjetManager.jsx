...
function ObjetNode({ node, wsId, token, onReload, depth = 0 }) {
  const [editing, setEditing] = useState(false)
  const [form, setForm] = useState({ nom: node.nom, nom_court: node.nom_court ?? '', est_individu: node.est_individu })
  ...
  return (
    <div className="jd-tree-node" style={{ marginLeft: depth ? '1.25rem' : 0 }}>
      <div className="jd-tree-row">
        ...
        {node.children.length === 0 && <span className="jd-tree-leaf">·</span>}
        ...
        {editing ? (
          <div className="jd-tree-edit">
            ...
            <label className="checkbox-row" style={{ fontSize: '.8rem' }}>
              <input type="checkbox" checked={form.est_individu} onChange={e => setForm(f => ({ ...f, est_individu: e.target.checked }))} />
              Individu
            </label>
          ...
          <div className="jd-tree-label">
            <span className={node.est_individu ? 'jd-individu' : 'jd-groupe'}>{node.nom}</span>
      ...
      {addingChild && (
        <div className="jd-tree-add" style={{ marginLeft: '1.5rem' }}>
          <input className="input" style={{ padding: '.25rem .5rem', fontSize: '.875rem', flex: 1 }}
            placeholder="Nom" value={childForm.nom} onChange={e => setChildForm(f => ({ ...f, nom: e.target.value }))} autoFocus />
          <input className="input" style={{ padding: '.25rem .5rem', fontSize: '.875rem', width: '80px' }}
            placeholder="court" value={childForm.nom_court} onChange={e => setChildForm(f => ({ ...f, nom_court: e.target.value }))} />
          <label className="checkbox-row" style={{ fontSize: '.8rem' }}>
            <input type="checkbox" checked={childForm.est_individu} onChange={e => setChildForm(f => ({ ...f, est_individu: e.target.checked }))} />
            Individu
          </label>
      ...
      {open && node.children.map(child => (
        <ObjetNode key={child.id} node={child} wsId={wsId} token={token} onReload={onReload} depth={depth + 1} />
      ))}
    </div>
  )
}
...
export default function ObjetManager() {
  ...
  async function addRoot() {
    ...
    await fetch(API_ROUTES.JD_OBJETS(wsId), {
      method: 'POST', headers: authHeader(token), body: JSON.stringify({ ...newForm, parent_id: null })
    })
  ...
  return (
    <div className="jd-manager">
      ...
      <div className="jd-tree">
        {tree.map(node => (
          <ObjetNode key={node.id} node={node} wsId={wsId} token={token} onReload={reload} />
        ))}
      </div>
